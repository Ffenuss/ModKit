...
DEX-инспектор допускает только доказанную двухинструкционную передачу константы
или поля в возвращаемый регистр без вызовов, записи и обработчиков исключений.
Тип и владелец поля проверяются. Частично обфусцированное имя геттера допускается,
если фактическое поле даёт классификационный сигнал. Это намеренно узкий первый
рецепт, а не полный анализ потока данных. Сложное тело остаётся кандидатом с причиной.
...
доступны через экспертный режим. Предварительный выбор всех рецептов удалён.
